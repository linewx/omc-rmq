...
class Management:
    def __init__(self, options):
        the_default_options = {"hostname": "localhost",
                               "port": "15672",
                               "path_prefix": "",
                               "declare_vhost": "/",
                               "username": "guest",
        ...
        tmp_options = the_default_options
        tmp_options.update(options)
        self.options = Obj(tmp_options)
        # self.args = args
    ...
    def invoke_help(self, args=None):
        if len(args) == 0:
            parser.print_help()
        else:
  ...
  config""")
    ...
    def invoke_publish(self, args=None):
        (uri, upload) = self.parse_args(args, EXTRA_VERBS['publish'])
    ...
    def invoke_get(self, args=None):
        (uri, upload) = self.parse_args(args, EXTRA_VERBS['get'])
